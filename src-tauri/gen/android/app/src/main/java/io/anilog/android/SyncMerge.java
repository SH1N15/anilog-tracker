...
final class SyncMerge {
    ...
    static Result merge(JSONObject localDocument, JSONObject remoteDocument) throws MergeException, org.json.JSONException {
        ...
        JSONArray following = new JSONArray();
        for (Long id : ids) {
            JSONObject winner = chooseRecord(localFollowing.get(id), remoteFollowing.get(id), "followedAt");
            ...
            if (localIntentAt > tombstone) {
                // A historical follow intent cannot bypass a newer deletion.
